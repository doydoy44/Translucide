...
error = function (txt, fadeout, focus) {
  popin(txt, fadeout, 'error', focus);
}
light = function (txt, fadeout, focus) {
  popin(txt, fadeout, 'light', focus);
}
...
edit_launcher = function (callback) {
  ...
  if (!$("#admin-bar").length) {
    $.ajax({
      url: path + "api/ajax.admin.php?mode=edit&type=" + type + "&id=" + id + "&date_update=" + date_update + (callback ? "&callback=" + callback : ""),
      cache: false
    })
      .done(function (html) {
        $("body").append(html);
...
$(function () {
  ...
  $(".bt.fixed.edit").on("mouseenter mouseleave focusin",
    function (event) {
      ...
      } else if (event.type == "mouseleave")// || event.type == "focusout"
      {
        hover_add = false;
        setTimeout(function () {
          if (!hover_add && !edit_on) $(".bt.fixed.add").fadeOut("fast");
        }, 1000);
      }
    }
  );
  ...
  $(".bt.fixed.add").hover(
    ...
    function () {
      hover_add = false;
      setTimeout(function () {
        if (!hover_add && !edit_on && $(".bt.fixed.edit").length) $(".bt.fixed.add").fadeOut("fast");
      }, 1000);
    });
  ...
  if (typeof state !== 'undefined' && state && state != "active" && get_cookie("auth").indexOf("edit-page") > 0) {
    ...
    $(".bt.fixed.construction").click(function () {
      $(this).slideUp();
    });
  }
