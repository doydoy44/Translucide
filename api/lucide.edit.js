...
save = function () //callback
{
  ...
  $(before_data).each(function (key, funct) {
    funct();
  });
  ...
  if (lucide == true)
    $.ajax({
      ...
      .done(function (html) {
        ...
        $(after_save).each(function (key, funct) {
          funct();
        });
...
exec_tool = function (command, value) {
  ...
  if (command) {
    ...
    if (command == "CreateAnchor") {
      var command_source = command;
      command = "CreateLink";
    }
...
img_position = function (align) {
  ...
  else if ($(memo_img).hasClass(align)) $(memo_img).removeClass(align);
...
unlucide = function () {
  ...
  $("#admin-bar").slideUp("400", function () {
    $(this).remove();
  });
...
$(function () {
  ...
  $("#admin-bar #date-insert").val($("meta[property='article:published_time']").last().attr("content").slice(0, 19).replace('T', ' ')).datepicker({
  ...
  $("#meta-responsive i").on("click", function () {
    $("#meta").addClass("tooltip slide-left fire pat").css({
      "position": "absolute",
      "top": $("#admin-bar").height()
    }).fadeToggle().attr('style', function (i, s) {
    ...
    $("#add-nav").on({
      "click.dragger": function (event) {
        ...
        if (event.target.className == "dragger") {
          ...
          $("#add-nav ul:not(.exclude) a").on("click", function () {
            return false;
          });
        }
      },
    ...
    $("#add-nav .zone, #add-nav .fa-cancel").on({
      "click": function (event) {
        ...
        if (!add_page_list) {
          ...
          var menu = {};
          $(document).find("header nav ul:not(.exclude) a").each(function (index) {
            menu[index] = $(this).attr('href');
          });
          ...
          $.ajax({
            ...
            success: function (html) {
              ...
              // désactive les liens
              $("#add-nav ul a").on("click", function () {
                return false;
              });
  ...
  if (typeof toolbox_videoLink != 'undefined') {
    videoLink = true;
    toolbox_video = true;
  } else videoLink = false;
  ...
  editable_event = function () {
    ...
    $(".editable").closest("a").on("click", function (event) {
      event.preventDefault();
    });
  ...
  $("#user i").on("click mouseenter",	// touchstart
    function (event) {
              ...
              function (event) {
                if (!$(event.target).parents().is("#user .absolute") && $("#user .absolute").is(":visible") && close == false)//event.type == 'click'
                  if ($("#user button.to-save").length || $("#user button i.fa-spin").length)// Si fiche pas sauvegardé on shake
                    ...
                    $("#user .absolute").fadeOut("fast", function () {
                      close = true;
                    });
      ...
      {
        close = true;
        $("#user .absolute").fadeIn("fast", function () {
          close = false;
        });
      ...
      buttons:
        [{
          ...
          click: function () {
            ...
            $(before_del).each(function (key, funct) {
              funct();
            });
            ...
            $.ajax({
              ...
              .done(function (html) {
                ...
                $(after_del).each(function (key, funct) {
                  funct();
                });
