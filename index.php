...
if ($res) {
    ...
    if ($http . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'] != make_url($res['url'], array_merge($GLOBALS['filter'], ["domaine" => true]))) {
        header($_SERVER['SERVER_PROTOCOL'] . " 301 Moved Permanently");
        header("location: " . make_url($res['url'], array_merge($GLOBALS['filter'], ["domaine" => true])));
        exit;
    }
    ...
    if ($res['state'] != "active") {// Page non activé
        ...
        if (!@$_SESSION['auth']['edit-' . $res['type']]) {
            ...
            if (isset($res_503['id'])) {
                ...
                $res['state'] = 'deactivate';
                if (@$close) {
                    $res['title'] = $msg = __("Site closing time");
                } else {
...
if (isset($GLOBALS['filter']['page'])) {
    $title .= ' - ' . __('Page') . ' ' . (int)$GLOBALS['filter']['page'];
}
...
<head>
    ...
    <?php if (isset($res['url'])) { ?>
        <meta property="og:url"
              content="<?= make_url($res['url'], array_merge($GLOBALS['filter'], ["domaine" => true])) ?>">
        <link rel="canonical" href="<?= make_url($res['url'], array_merge($GLOBALS['filter'], ["domaine" => true])) ?>">
    <?php } ?>
    <?php if ($description) { ?>
...
<?php
    ...
    <script>console.log("<?=benchmark()?>")</script><?php }
